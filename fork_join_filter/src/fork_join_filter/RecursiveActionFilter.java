...
public class RecursiveActionFilter extends RecursiveAction {
    ...
    protected void compute() {
        if (measure() < THRESHOLD) {
            ...
            for (int i = 1; i < original.rows() - 1; i++) {
                edgeInterpolation(i, r);
                edgeInterpolation(i, g);
                edgeInterpolation(i, b);
            }
            ...
            RecursiveActionFilter leftFilter = new RecursiveActionFilter(original.submat(new Range(0, original.rows()), new Range(0, original.cols() / 2)),
                    filtered.submat(new Range(0, filtered.rows()), new Range(0, filtered.cols() / 2)));
            RecursiveActionFilter rightFilter = new RecursiveActionFilter(original.submat(new Range(0, original.rows()), new Range(original.cols() / 2,
                    original.cols())), filtered.submat(new Range(0, filtered.rows()), new Range(filtered.cols() / 2, filtered.cols())));
    ...
    private void edgeInterpolation(int i, Mat r) {
        r.put(i, 0, (r.get(i - 1, 1)[0] * 2 + r.get(i, 1)[0] * 4 + r.get(i + 1, 1)[0] * 2) / 7.75);
        r.put(i, original.cols() - 1, (r.get(i - 1, original.cols() - 2)[0] * 2 + r.get(i, original.cols() - 2)[0]
                * 4 + r.get(i + 1, original.cols() - 2)[0] * 2) / 7.75);
    }
